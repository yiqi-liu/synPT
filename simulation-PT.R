...
library(CVXR)
library(synthdid)
# setwd("~/Desktop/synPT/simulation")
source("all-func.R")
set.seed(0)

# read repeated cross-sectional data; see the first chunk of code for how this data is generated
...
# collect results; type in c("est", "se", "coverage", "cs_lb", "cs_ub", "time", "est_lb", "est_ub"); DGP in c("PT1", "noPT1", "SDID1", "PT10", "noPT10", "SDID10")
result_pt <- data.frame(matrix(ncol=42, nrow = 0))
colnames(result_pt) <- c(1979:2016, 2018, "type", "DGP", "niter")
result_pt <- result_pt %>%
  mutate(type = as.character(type), DGP = as.character(DGP))
...
for (iter in 1:num_MC){
  ...
  start <- Sys.time()
  ### START: DGP-PT1 treated unit is one state, PT is satisfied ----
  ...
  # # renumber the treated unit to be unit 1, and the rest are control units
  # rcs_data <- rcs_data %>% mutate(K = dense_rank(desc(G_i)))
  ...
  result_pt <- bind_rows(
    result_pt,
    c(as.list(tau_pt), list(type = "est", DGP = "PT1", niter=iter)),
    c(as.list(se_pt), list(type = "se", DGP = "PT1", niter=iter)),
    c(as.list(coverage_pt), list(type = "coverage", DGP = "PT1", niter=iter)),
    c(as.list(time_elapsed_pt), list(type = "time", DGP = "PT1", niter=iter)),
    c(as.list(diff_in_means), list(type = "est_dim", DGP = "PT1", niter=iter)),
    c(as.list(se_diff_in_means), list(type = "se_dim", DGP = "PT1", niter=iter)),
    c(as.list(cov_diff_in_means), list(type = "coverage_dim", DGP = "PT1", niter=iter))
  )
  ## END: PT-DID ------------------------------------
  ...
  result_sdid <- bind_rows(
    result_sdid,
    list("2018" = tau_sdid, type = "est", DGP = "PT1", niter=iter),
    list("2018" = se_sdid, type = "se", DGP = "PT1", niter=iter),
    list("2018" = coverage_sdid, type = "coverage", DGP = "PT1", niter=iter),
    list("2018" = time_elapsed_sdid, type = "time", DGP = "PT1", niter=iter)
  )
  ## END: SDID --------------------------------------
  ...
  result_sc <- bind_rows(
    result_sc,
    list("2018" = tau_sc, type = "est", DGP = "PT1", niter=iter),
    list("2018" = se_sc, type = "se", DGP = "PT1", niter=iter),
    list("2018" = coverage_sc, type = "coverage", DGP = "PT1", niter=iter),
    list("2018" = time_elapsed_sc, type = "time", DGP = "PT1", niter=iter)
  )
  ## END: SC ------------------------------------
  ...
  b <- mid+len
  cand_tau_PT <- a+(b-a)*
    qbeta(seq(0, 1, length.out = 500), shape1 = 2, shape2 = 2)
  
  # collect results
  result_spt <- bind_rows(
    result_spt,
    list("2018" = min(cs_tau), type = "cs_lb", DGP = "PT1", niter=iter),
    list("2018" = max(cs_tau), type = "cs_ub", DGP = "PT1", niter=iter),
    list("2018" = as.numeric(true_tau <= max(cs_tau) & true_tau >= min(cs_tau)), type = "coverage", DGP = "PT1", niter=iter),
    list("2018" = time_elapsed_spt, type = "time", DGP = "PT1", niter=iter)
  )
  frac_rej_PT <- append(frac_rej_PT, 1-length(cs_tau)/500)
  ## END: SPT ------------------------------------
  ### END: DGP-PT1 treated unit is one state, PT is satisfied ----
  
  
  ## START: DGP-noPT1 treated unit is one state, PT is satisfied for pre-trends but not for post-trend ----
  ...
  result_pt <- bind_rows(
    result_pt,
    c(as.list(tau_pt), list(type = "est", DGP = "noPT1", niter=iter)),
    c(as.list(se_pt), list(type = "se", DGP = "noPT1", niter=iter)),
    c(as.list(coverage_pt), list(type = "coverage", DGP = "noPT1", niter=iter)),
    c(as.list(time_elapsed_pt), list(type = "time", DGP = "noPT1", niter=iter)),
    c(as.list(diff_in_means), list(type = "est_dim", DGP = "noPT1", niter=iter)),
    c(as.list(se_diff_in_means), list(type = "se_dim", DGP = "noPT1", niter=iter)),
    c(as.list(cov_diff_in_means), list(type = "coverage_dim", DGP = "noPT1", niter=iter))
  )
  ## END: PT-DID ------------------------------------
  ...
  result_sdid <- bind_rows(
    result_sdid,
    list("2018" = tau_sdid, type = "est", DGP = "noPT1", niter=iter),
    list("2018" = se_sdid, type = "se", DGP = "noPT1", niter=iter),
    list("2018" = coverage_sdid, type = "coverage", DGP = "noPT1", niter=iter),
    list("2018" = time_elapsed_sdid, type = "time", DGP = "noPT1", niter=iter)
  )
  ## END: SDID --------------------------------------
  ...
  result_sc <- bind_rows(
    result_sc,
    list("2018" = tau_sc, type = "est", DGP = "noPT1", niter=iter),
    list("2018" = se_sc, type = "se", DGP = "noPT1", niter=iter),
    list("2018" = coverage_sc, type = "coverage", DGP = "noPT1", niter=iter),
    list("2018" = time_elapsed_sc, type = "time", DGP = "noPT1", niter=iter)
  )
  ## END: SC ------------------------------------
  ...
  result_spt <- bind_rows(
    result_spt,
    list("2018" = min(cs_tau), type = "cs_lb", DGP = "noPT1", niter=iter),
    list("2018" = max(cs_tau), type = "cs_ub", DGP = "noPT1", niter=iter),
    list("2018" = as.numeric(true_tau <= max(cs_tau) & true_tau >= min(cs_tau)), type = "coverage", DGP = "noPT1", niter=iter),
    list("2018" = time_elapsed_spt, type = "time", DGP = "noPT1", niter=iter)
  )
  frac_rej_noPT <- append(frac_rej_noPT, 1-length(cs_tau)/500)
  ## END: SPT ------------------------------------
  ### END: DGP-noPT1 treated unit is one state, PT is satisfied for pre-trends but not for post-trend ----
  
  end <- Sys.time()
  
  print(paste0("PT1: ----------------------"))
  print(paste0("PT-DID avg bias: ", mean(abs(true_tau-subset(result_pt, type=="est" & DGP=="PT1")$`2018`))))
  print(paste0("PT-DID avg bias (DiM): ", mean(abs(true_tau-subset(result_pt, type=="est_dim" & DGP=="PT1")$`2018`))))
  print(paste0("PT-DID avg CI length: ", 1.96*2*mean(subset(result_pt, type=="se" & DGP=="PT1")$`2018`)))
  print(paste0("PT-DID avg CI length (DiM): ", 1.96*2*mean(subset(result_pt, type=="se_dim" & DGP=="PT1")$`2018`)))
  print(paste0("PT-DID coverage rate: ", mean(subset(result_pt, type=="coverage" & DGP=="PT1")$`2018`)))
  print(paste0("PT-DID coverage rate (DiM): ", mean(subset(result_pt, type=="coverage_dim" & DGP=="PT1")$`2018`)))
  print(paste0("PT-DID avg time: ", mean(subset(result_pt, type=="time" & DGP=="PT1")$`2018`)))
  print(paste0("-"))
  print(paste0("SDID avg bias: ", mean(abs(true_tau-subset(result_sdid, type=="est" & DGP=="PT1")$`2018`))))
  print(paste0("SDID avg CI length: ", 1.96*2*mean(subset(result_sdid, type=="se" & DGP=="PT1")$`2018`)))
  print(paste0("SDID coverage rate: ", mean(subset(result_sdid, type=="coverage" & DGP=="PT1")$`2018`)))
  print(paste0("SDID avg time: ", mean(subset(result_sdid, type=="time" & DGP=="PT1")$`2018`)))
  print(paste0("-"))
  print(paste0("SC avg bias: ", mean(abs(true_tau-subset(result_sc, type=="est" & DGP=="PT1")$`2018`))))
  print(paste0("SC avg CI length: ", 1.96*2*mean(subset(result_sc, type=="se" & DGP=="PT1")$`2018`)))
  print(paste0("SC coverage rate: ", mean(subset(result_sc, type=="coverage" & DGP=="PT1")$`2018`)))
  print(paste0("SC avg time: ", mean(subset(result_sc, type=="time" & DGP=="PT1")$`2018`)))
  print(paste0("-"))
  print(paste0("SPT avg CS length: ", mean(subset(result_spt, type=="cs_ub" & DGP=="PT1")$`2018`-subset(result_spt, type=="cs_lb" & DGP=="PT1")$`2018`)))
  print(paste0("SPT coverage rate: ", mean(subset(result_spt, type=="coverage" & DGP=="PT1")$`2018`)))
  print(paste0("SPT avg time: ", mean(subset(result_spt, type=="time" & DGP=="PT1")$`2018`)))
  print(paste0("frac cand_tau rej: ", mean(frac_rej_PT)))
  print(paste0("noPT1: ----------------------"))
  print(paste0("avg dev from parallel posttrend: ", mean(violation_noPT)))
  print(paste0("PT-DID avg bias: ", mean(abs(true_tau-subset(result_pt, type=="est" & DGP=="noPT1")$`2018`))))
  print(paste0("PT-DID avg bias (DiM): ", mean(abs(true_tau-subset(result_pt, type=="est_dim" & DGP=="noPT1")$`2018`))))
  print(paste0("PT-DID avg CI length: ", 1.96*2*mean(subset(result_pt, type=="se" & DGP=="noPT1")$`2018`)))
  print(paste0("PT-DID avg CI length (DiM): ", 1.96*2*mean(subset(result_pt, type=="se_dim" & DGP=="noPT1")$`2018`)))
  print(paste0("PT-DID coverage rate: ", mean(subset(result_pt, type=="coverage" & DGP=="noPT1")$`2018`)))
  print(paste0("PT-DID coverage rate (DiM): ", mean(subset(result_pt, type=="coverage_dim" & DGP=="noPT1")$`2018`)))
  print(paste0("PT-DID avg time: ", mean(subset(result_pt, type=="time" & DGP=="noPT1")$`2018`)))
  print(paste0("-"))
  print(paste0("SDID avg bias: ", mean(abs(true_tau-subset(result_sdid, type=="est" & DGP=="noPT1")$`2018`))))
  print(paste0("SDID avg CI length: ", 1.96*2*mean(subset(result_sdid, type=="se" & DGP=="noPT1")$`2018`)))
  print(paste0("SDID coverage rate: ", mean(subset(result_sdid, type=="coverage" & DGP=="noPT1")$`2018`)))
  print(paste0("SDID avg time: ", mean(subset(result_sdid, type=="time" & DGP=="noPT1")$`2018`)))
  print(paste0("-"))
  print(paste0("SC avg bias: ", mean(abs(true_tau-subset(result_sc, type=="est" & DGP=="noPT1")$`2018`))))
  print(paste0("SC avg CI length: ", 1.96*2*mean(subset(result_sc, type=="se" & DGP=="noPT1")$`2018`)))
  print(paste0("SC coverage rate: ", mean(subset(result_sc, type=="coverage" & DGP=="noPT1")$`2018`)))
  print(paste0("SC avg time: ", mean(subset(result_sc, type=="time" & DGP=="noPT1")$`2018`)))
  print(paste0("-"))
  print(paste0("SPT avg CS length: ", mean(subset(result_spt, type=="cs_ub" & DGP=="noPT1")$`2018`-subset(result_spt, type=="cs_lb" & DGP=="noPT1")$`2018`)))
  print(paste0("SPT coverage rate: ", mean(subset(result_spt, type=="coverage" & DGP=="noPT1")$`2018`)))
  print(paste0("SPT avg time: ", mean(subset(result_spt, type=="time" & DGP=="noPT1")$`2018`)))
  print(paste0("frac cand_tau rej: ", mean(frac_rej_noPT)))
  
  print(end-start)
  
  write.csv(result_spt, "result_spt_2se.csv", row.names=FALSE)
  write.csv(result_pt, "result_pt_2se.csv", row.names=FALSE)
  write.csv(result_sc, "result_sc_2se.csv", row.names=FALSE)
  write.csv(result_sdid, "result_sdid_2se.csv", row.names=FALSE)
}