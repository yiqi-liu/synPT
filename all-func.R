...
# DGP-PT:   PT==TRUE  & SDID==FALSE
# DGP-noPT: PT==FALSE & SDID==FALSE
# DGP-SDID: PT==FALSE & SDID==TRUE
gen_micro_data_control <- function(k, t=NULL, n, 
  ...
  if (!SDID){
    if (!PT & t==2018){ # if DGP-noPT1, we generate post-treatment outcomes differently
...
# generate population means for the treated unit; only used for DGP-PT and DGP-noPT
gen_trt_unit <- function(trt_idx, pi_0=NULL, PT=TRUE){
  ...
  if (!PT){ # generate nonparallel posttrend in the noPT DGP 
...
# generate fake repeated cross-sectional sample for the treated unit
...
tau_set <- function(outcome,  # 1 by n vector of outcomes
  ...
  # Rewrite (Aw-b)'(Aw-b)+(aw-d+tau) = w'(A'A+aa')w + w'(-2A'b-2ad+2a*tau) + (const independent of w) = w'Pw/2 + (q_base+2a*tau)'w + const for
  # P = 2(A'A+aa'); q_base = 2(-A'b-ad); let q(tau) = q_base + 2a*tau
...
# function that create candidate values of tau by running an initial LP
...
## function to find a different convex weight that generates parallel pretrends but not parallel posttrends:
